	...
	function single_reply ($eachreply, $i=0) {
		...
		if ($eachreply['reproperty']==0 || $eachreply['reproperty']==1) { // A Normal Reply
			...
			if ($eachreply['replierid']==-1) {
				...
				$replier="<a class='replaiers'  style='text-decoration:none'>{$eachreply['replier']}</a>";
				//Avatars for users
				if ($flset['avatar']!=1) {
			...
			if ($userdetail['ip']==$eachreply['repip'] && ($nowtime['timestamp']-$eachreply['reptime']<$mbcon['editcomment']) && $eachreply['reppsw']=='' && empty($eachreply['adminreptime'])) { 			//Allow edit
				...
				$replycontent.="<div id=\"editcomment{$eachreply['repid']}\" style='display: none;'><form action=\"javascript: ajax_editcomment({$eachreply['repid']}, 'reply', {$i});\" method=\"post\" id=\"formeditcomment{$eachreply['repid']}\"><div class='ds-textarea-wrapper ds-rounded-top'><textarea cols='44' rows='10' class='comment_cont'  id=\"editcomcontent{$eachreply['repid']}\" name='v_contentc'>{$rawreplycontent}</textarea></div>&nbsp;&nbsp;&nbsp;&nbsp;<input type='button' value='{$lnc[25]}' onclick=\"ajax_editcomment({$eachreply['repid']}, 'reply', {$i});\" class='button' /> <input type='reset' value='{$lnc[26]}' class='button' /></form></div>";
			}
			...
			if ($eachreply['adminreptime'] && ($eachreply['reproperty']!=1 || $permission['SeeHiddenReply']==1 || $eachreply['reppsw']=='')) {
				...
				$adminreplier="<a  title=\"{$lnc[17]}\">{$eachreply['adminreplier']}</a>";
				...
				$adminreplybody.="<span class='replayits'>{$lnc[24]} </span><br/><div class='ds-textarea-wrapper ds-rounded-top adds'><textarea cols='66' rows='3' name='adminreplycontent' id='adminreplycontent{$eachreply['repid']}'>".safe_invert($eachreply['adminrepcontent'])."</textarea></div>";
			...
			$output_single="<div id=\"blogcomment{$eachreply['repid']}\" class='hybreplay'>".$output_single."</div>";
